        ...
        <div class="detailDiv" v-if="proItem.proDetailArray.length!=0">
            ...
            <div class="detailTitleDiv">
                <div class="detailTitleContentDiv">
                    <div class="detailTitleFirstWhite"></div>
                    <div v-for="(title,index) in this.proDetailTitleName" class="title_item" :key="index">
                        <div v-if="index==9" >
                            <div class="title_item9">{{title}}</div>
                        </div>
                        <div v-else-if="index==10" >
                            <div class="title_item10">{{title}}</div>
                        </div>
                        <div v-else >
                            <div class="title_item_other">{{title}}</div>
                        </div>
                    </div>
                    <div class="detailTitleFirstWhite"></div><br/>

                    <div v-for="(detail_item,index) in proItem.proDetailArray" class="detail_item" :key="detail_item.id">
                        <div class="detailItemFirstWhite"></div>
                        ...
                        <div class="editDiv">
                          <i class="iconfont editIcon" @click="editThisPro(proItem,index.toString())"></i>
                        </div>
                        ...
                        <div class="detailItemLastWhite"></div>             
                    </div>
                </div>
...
export default {
  ...
  methods: {
      ...
      editThisPro(proItemm,detaddd){
                // this.currenSelectedItem = item;
                console.log("index = "+detaddd);
      var message = JSON.stringify(proItemm); //对象转为字符串
      var lStorage = window.localStorage;
      lStorage.proDetailItem = message;
      
      var index =  JSON.stringify(detaddd);
      lStorage.proDetailIndex = index;

      this.$router.push("/order/carEditDetail");
    ...
    .detailDiv
      ...
      .detailTitleDiv
          ...
          .detailTitleContentDiv
              background #ffffff
              width 300%
              .detailTitleFirstWhite
                background #f2f2f2
                width 4%
                height 0.3rem
                float left
              .detailItemFirstWhite                          
                background #f2f2f2
                width 4%
                height 0.3rem
                float left                
